// ArenaRoom — PvP arena: duel / team / FFA, rounds, optional PvE.
// Lifecycle: LOBBY -> COUNTDOWN -> PLAYING -> (round win -> reset -> COUNTDOWN) -> GAMEOVER
// Honors the same security boundaries as GameRoom (input validation, rate
// limits, onAuth token bucket). Extra arena invariants:
//   - mode sanitized via src/shared/arena.js
//   - team assignment via assignTeams (duel/team/FFA)
//   - rounds: first to roundsToWin wins match; per-round score target is arenaTargetScore
//   - PvE toggle: when false, enemies stay dead (no wave AI); when true, wave system runs as in GameRoom
//   - friendly fire disabled in team mode (teammates cannot damage each other)
//   - PvP kills award SERVER.arena.killScore (10) and XP
...
export default class ArenaRoom extends Room {
  // Hard cap: per-mode (duel 2, team/ffa up to 12); updated in onCreate from options.
  maxClients = SERVER.arena?.ffa?.maxPlayers ?? 12;
  ...
  onCreate(options = {}) {
    ArenaRoom.instances.add(this);
    // Arena options (from lobby matchMaker or direct join)
    this.arenaMode = sanitizeMode(options.mode ?? options.arenaMode ?? SERVER.arena?.defaultMode ?? 'ffa');
    this.pveEnabled = sanitizePve(options.pve ?? options.arenaPve ?? SERVER.arena?.pveDefault ?? false);
    this.roundsToWin = sanitizeRoundsToWin(options.roundsToWin ?? SERVER.arena?.roundsToWin ?? 2);
    this.roundTargetScore = Math.max(1, Math.floor(Number(options.roundTargetScore ?? options.targetScore ?? SERVER.arena?.roundTargetScore ?? 30)) || 30);
    // clamp maxClients to mode limit so a duel room never fills with 12
    const maxForMode = maxPlayersForMode(this.arenaMode);
    if (maxForMode) this.maxClients = maxForMode;
    // lockAfterSecs is not needed; maxClients + explicit lock via this.lock() if needed
    ...
    // Arena metadata broadcast to clients
    this.state.arenaMode = this.arenaMode;
    this.state.arenaPve = this.pveEnabled;
    this.state.arenaRoundsToWin = this.roundsToWin;
    this.state.arenaRound = 1;
    this.state.arenaTargetScore = this.roundTargetScore;
    // arenaRoundWins stays empty until first round win
    ...
    this._roundWins = new Map(); // internal round win tracker: key string -> wins
    ...
    this.clock.setInterval(() => {
      ...
      this.update(dt);
      ArenaRoom.stats.lastTickMs = performance.now() - t0;
    }, SERVER.tickMs);

    this.logEvent('arena_create', { mode: this.arenaMode, pve: this.pveEnabled, roundsToWin: this.roundsToWin, targetScore: this.roundTargetScore });
  }
  ...
  spawnEnemies() {
    ...
    if (this.pveEnabled) {
      ...
      // Pure PvP: all enemies stay dead (hp 0) — no waves
      this.state.enemies.forEach((e) => { e.hp = 0; });
      this.state.wave = 1;
    }
  }
  ...
  onJoin(client, options = {}) {
    // Spectator (PRD-arena-spectate.md): watch-only connection. No PlayerState
    // seat, no capacity rejection (spectators allowed even when full), no team
    // rebalance, no auto-start/auto-restart side effects — presence only.
    ...
    if (player) {
      ...
      if (this.state.players.size >= maxForMode) {
        this.warnEvent('join_rejected_full', { sid: client.sessionId, mode: this.arenaMode, max: maxForMode });
        throw new Error(`arena full for ${this.arenaMode} (max ${maxForMode})`);
      }
      ...
      this.logEvent('player_join', { sid: client.sessionId, name, players: this.state.players.size, mode: this.arenaMode });
    }
    ...
    // Arena match start: countdown when enough players for the mode
    const minForMode = minPlayersForMode(this.arenaMode);
    if (this.state.matchState === 'lobby' && this.state.players.size >= minForMode) {
      this.startCountdown();
    } else if (this.state.matchState === 'gameover' && this.state.players.size === 1) {
      // JOIN DURING GAME_OVER (empty arena): fresh countdown alone rather than stuck waiting
      this.logEvent('match_auto_restart', { sid: client.sessionId, reason: 'join_during_gameover' });
      this.resetMatch();
    } else if (this.state.matchState === 'gameover' && this.state.players.size >= minForMode) {
      // Gameover but now enough players queued via lobby? Also auto-restart
      this.logEvent('match_auto_restart', { sid: client.sessionId, reason: 'join_during_gameover_enough' });
      this.resetMatch();
    }
  }

  /** Assign team ids to all current players based on mode (deterministic). */
  assignArenaTeams() {
    const sids = [...this.state.players.keys()];
    // Sort to ensure deterministic assignment (join order may vary, but Map order is insertion order)
    const assignment = assignTeams(sids, this.arenaMode);
    this._teamAssignment = assignment;
    for (const [sid, team] of assignment) {
      const p = this.state.players.get(sid);
      if (p) p.team = team;
      this.state.arenaRoundWins; // keep schema in sync? roundWins map uses string keys
    }
    // For team mode, repaint colors by team for visual clarity (optional)
    if (this.arenaMode === 'team') {
      const teamColors = [0x90caf9, 0xf48fb1]; // team 0 blueish, team 1 pinkish
      for (const [sid, team] of assignment) {
        const p = this.state.players.get(sid);
        if (p) p.color = teamColors[team % teamColors.length];
      }
    }
    this.logEvent('teams_assigned', { mode: this.arenaMode, assignment: [...assignment.entries()] });
  }
  ...
  onLeave(client, code = CloseCode.CONSENTED) {
    ...
    // Spectator (PRD-arena-spectate.md): no seat exists — presence removal is
    // all that's needed. Skip seat cleanup / team rebalance / grace-reconnect
    // paths that assume a player state existed.
  ...
  removePlayer(sid) {
    ...
    this._teamAssignment?.delete(sid);
    ...
    // Re-assign teams when someone leaves (keeps FFA indices compact, team balance)
    if (this.state.players.size > 0) this.assignArenaTeams();
  }
  ...
  resetMatch() {
    ...
    state.winnerId = '';
    state.winnerName = '';
    // Arena: reset round accounting
    this._roundWins = new Map();
    state.arenaRoundWins.clear();
    state.arenaRound = 1;
    ...
    // Fresh match = wave 1 if PvE, else keep enemies dead.
    this.pendingMelee = [];
    this.state.projectiles.clear();
    if (this.pveEnabled) this.spawnWave(1);
    else this.state.enemies.forEach((e) => { e.hp = 0; });
    ...
    this.assignArenaTeams();
    this.logEvent('match_reset', { mode: this.arenaMode, round: state.arenaRound });
    this.startCountdown();
  }
  ...
  onNextWave(client) {
    if (!this.pveEnabled) return; // pure PvP has no waves
  ...
  /** True when attacker and victim are on the same team in team mode (no friendly fire). */
  isFriendlyFire(attackerSid, victimSid) {
    if (this.arenaMode !== 'team') return false;
    const a = this.state.players.get(attackerSid);
    const v = this.state.players.get(victimSid);
    if (!a || !v) return false;
    if (a.team === -1 || v.team === -1) return false;
  ...
  awardPvpKill(killerSid, victimSid) {
    ...
    const victim = this.state.players.get(victimSid);
    if (!killer || !victim) return;
    const killScore = SERVER.arena?.killScore ?? 10;
  ...
  melee(sid) {
    ...
    // PvP: same swing also hurts other living players in the arc (team-aware).
    for (const [osid, victim] of this.state.players) {
      if (osid === sid || victim.hp <= 0) continue;
      if (this.isFriendlyFire(sid, osid)) continue;
      if (meleeHits(player, [victim], cfg).length) {
        const hpBefore = victim.hp;
        const damaged = this.damagePlayer(osid, victim, pvpDmg, player.x, player.z);
        if (damaged && victim.hp <= 0 && hpBefore > 0) {
          this.awardPvpKill(sid, osid);
        }
      }
    }
  }
  ...
  castSkill(sid) {
    ...
    // Only process enemy hits when PvE is enabled.
    if (this.pveEnabled && result.hits.length > 0) {
    ...
    // PvP: apply the pre-resolved victim list (per-class damage, team-aware, kill scoring).
    for (const osid of pvpVictims) {
      const victim = state.players.get(osid);
      if (victim) {
        const hpBefore = victim.hp;
        const damaged = this.damagePlayer(osid, victim, skillPvpDmg, player.x, player.z);
        if (damaged && victim.hp <= 0 && hpBefore > 0) {
          this.awardPvpKill(sid, osid);
        }
      }
  ...
  updateProjectiles(dt) {
    ...
    for (let i = state.projectiles.length - 1; i >= 0; i--) {
      ...
      // Hit enemies (only when PvE enabled)
      if (proj.ownerIsPlayer && this.pveEnabled) {
      ...
      // PvP: hit other players (team-aware + kill scoring)?
      if (!removed && proj.ownerIsPlayer) {
        for (const [osid, victim] of state.players) {
          if (osid === proj.ownerSid || victim.hp <= 0) continue;
          if (this.isFriendlyFire(proj.ownerSid, osid)) continue;
          if (projectileHitsTarget(proj, victim, hitRadius)) {
            const hpBefore = victim.hp;
            const damaged = this.damagePlayer(osid, victim, proj.damage, proj.x, proj.z);
            if (damaged && victim.hp <= 0 && hpBefore > 0) {
              this.awardPvpKill(proj.ownerSid, osid);
            }
            state.projectiles.splice(i, 1);
            removed = true;
            break;
  ...
  updatePlaying(dt) {
    ...
    // Only when PvE is enabled; pure PvP has no enemies.
    let alive = 0;
    if (this.pveEnabled) {
      for (const enemy of state.enemies) {
  ...
  /** Arena round win: first player/team to reach roundTargetScore wins the round; first to roundsToWin wins match. */
  checkWinConditions(dt) {
    // Use arena helpers to determine round winner (living players only).
    const winner = roundWinner(this.state.players, this.arenaMode, this._teamAssignment, this.roundTargetScore);
    if (!winner) return;
    // Record round win
    if (winner.kind === 'team') {
      const key = String(winner.teamId);
      const cur = this._roundWins.get(key) || 0;
      this._roundWins.set(key, cur + 1);
      this.state.arenaRoundWins.set(key, cur + 1);
      this.logEvent('round_win', { mode: this.arenaMode, team: winner.teamId, round: this.state.arenaRound, wins: cur + 1 });
      // Check match winner
      const mWinner = matchWinner(this._roundWins, this.roundsToWin);
      if (mWinner !== null) {
        // Team match win: pick a representative player from winning team as winnerId for display
        let repSid = '';
        let repName = `Team ${winner.teamId}`;
        for (const [sid, team] of this._teamAssignment) {
          if (team === winner.teamId) {
            const p = this.state.players.get(sid);
            if (p) { repSid = sid; repName = p.name; break; }
          }
        }
        this.endMatch(repSid || [...this._teamAssignment.keys()][0] || '');
        // Store team winner name if needed
        if (repName.startsWith('Team')) this.state.winnerName = repName;
        return;
      }
    } else {
      const key = winner.sid;
      const cur = this._roundWins.get(key) || 0;
      this._roundWins.set(key, cur + 1);
      this.state.arenaRoundWins.set(key, cur + 1);
      this.logEvent('round_win', { mode: this.arenaMode, sid: key, round: this.state.arenaRound, wins: cur + 1 });
      const mWinner = matchWinner(this._roundWins, this.roundsToWin);
      if (mWinner !== null) {
        this.endMatch(String(mWinner));
        return;
      }
    }
    // Round won but match not over → reset for next round
    this.resetRound();
  }

  /** Reset for next round: positions, hp, scores, cooldowns wiped, next countdown. Keeps roundWins. */
  resetRound() {
    const state = this.state;
    state.arenaRound += 1;
    this.logEvent('round_reset', { round: state.arenaRound, mode: this.arenaMode, wins: [...this._roundWins.entries()] });
    for (const player of state.players.values()) {
      const p = this.randomPos();
      player.x = p.x;
      player.z = p.z;
      ...
      // Do NOT clear level/xp/upgrades — arena rounds keep progression within a match
    }
    for (const sid of state.players.keys()) {
      this.inputs.set(sid, { dirX: 0, dirZ: 0 });
      this.attackAt.set(sid, 0);
    ...
    // Respawn orbs/powerUps, clear projectiles, reset waves if needed
    for (const orb of state.orbs) {
      const p = this.randomPos();
      orb.x = p.x;
      orb.z = p.z;
    ...
    this.state.projectiles.clear();
    this.matchElapsed = 0;
    // Keep winnerId/winnerName empty until match over
    state.winnerId = '';
    state.winnerName = '';
    this.startCountdown();
  }
