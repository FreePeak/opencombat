// Server-side tunables — the single place the server reads gameplay numbers
// from. The client has its own src/config.js for visuals; anything that
// affects the simulation lives here because the server is authoritative.
export const SERVER = {
  port: 2567,

  // Fixed timestep: the room updates the simulation every tickMs and
  // integrates movement with dt = tickMs/1000, so the game runs at the
  // same speed regardless of network or render rate.
  tickMs: 50,
  ...
  // Design choice (documented in README): with minPlayers = 1 the
  // countdown starts as soon as the first player joins — no waiting for
  // friends, the lobby phase is just the moment before the first join.
  match: {
    minPlayers: 1,
