...
| 1.3 Extract shared-sim duplication D2-D8 | Q1 | Pull SAFE-TO-EXTRACT mirrored blocks (progression bookkeeping, shop effects, burn DoT, enemy-hit resolution, projectile loop, pause wall, match reset) into shared modules used by GameRoom + LocalRoom | FR-FAIR-03, FR-CORE-05 | IN PROGRESS | feat/p1.3-slice1-leveling (slice 1/4 done) | 9c7f89f | Red-first TDD: `node --test test/simLeveling.test.mjs` fail (ERR_MODULE_NOT_FOUND) -> 16/16 pass. Full gate: `npm run check` 67 files ok; `npm test` 19 files / 58 tests pass (was 18/42); `npm run smoke` OK — 8/8 incl. live WebSocket join. LOC: GameRoom.js 1330 -> 1240 (-90), LocalRoom.js 892 -> 852 (-40); new src/shared/sim/leveling.js 196 lines. Notes: design-doc line ranges matched HEAD 321c22c (no drift); LocalRoom scalars migrated to sid-keyed Maps per plan, with a `_pendingUntil` scalar accessor kept because phase4.test.mjs:365 assigns it directly | 2026-08-22 |
